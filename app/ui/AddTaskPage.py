...
class AddTaskWindow(tk.Tk):
    def __init__(self, main_app):
        ...
        try:
            with sqlite3.connect(self.tags_path) as conn:
                c = conn.cursor()
                c.execute("CREATE TABLE IF NOT EXISTS tags (tag_name TEXT PRIMARY KEY)")
                
                # Check if tags table is empty and add some default tags if needed
                c.execute("SELECT COUNT(*) FROM tags")
                if c.fetchone()[0] == 0:
                    default_tags = ["Work", "Personal", "Urgent", "Important", "Low Priority"]
                    for tag in default_tags:
                        try:
                            c.execute("INSERT INTO tags VALUES (?)", (tag,))
                        except sqlite3.IntegrityError:
                            pass  # Skip if tag already exists
                    conn.commit()
                    
                # Load tags for the UI
                c.execute("SELECT tag_name FROM tags")
                tags = c.fetchall()
                self.values = [tag[0] for tag in tags]
                
        except sqlite3.Error as e:
            messagebox.showerror("Database Error", f"Failed to load tags:\n{e}")
            self.values = []
        ...
        self.tag_text = tk.Text(tag_frame, height=7, width=12, bg='#d3d3d3', relief="solid", bd=1, font=("SF Pro Text", 10), yscrollcommand=tag_scrollbar.set)
        ...
        self.tag_listbox = tk.Listbox(listbox_frame, selectmode="multiple", exportselection=0, width=14, height=8, bg="#d3d3d3", relief='solid', yscrollcommand=list_scrollbar.set)
        ...
        autofill_date_btn = tk.Button(button_frame, text="Autofill date", command=self.autofill_date, bg="#E39ff6", fg="#000000", font=("SF Pro Text", 10), activebackground="#800080", activeforeground="#000000", width=button_width)
    ...
    def update_tag_entry(self):
        ...
        selected_values = [self.tag_listbox.get(idx) for idx in selected_indices]
    ...
    def confirm_action(self):
        ...
        check_field = {"TaskName": task_name, "Start Date": start_date, "Complexity Type": complexity_type, "Complexity Value": complexity_value}
        ...
        if tags:
            res = tags.split('\n')
            if not all(tag in self.values for tag in res):
        ...
        # Add task if confirmed
        if confirm:
            try:
                # Make sure we're using a writable database path
                writable_db_path = self.path
                
                # If we're running from PyInstaller, make sure we're using a writable location
                if hasattr(sys, '_MEIPASS'):
                    if os.name == 'nt':  # Windows
                        data_dir = os.path.join(os.environ['APPDATA'], 'TimeTracker')
                    else:  # macOS and Linux
                        data_dir = os.path.join(os.path.expanduser('~'), '.timetracker')
                    
                    os.makedirs(data_dir, exist_ok=True)
                    writable_db_path = os.path.join(data_dir, 'task_list.db')
                    
                    # If the database doesn't exist in the writable location, copy or create it
                    if not os.path.exists(writable_db_path):
                        try:
                            if os.path.exists(self.path):
                                shutil.copy2(self.path, writable_db_path)
                            else:
                                # Create a new database
                                temp_conn = sqlite3.connect(writable_db_path)
                                temp_conn.close()
                        except Exception as e:
                            messagebox.showerror("File Error", f"Could not create writable database: {e}")
                            return
                ...
                # Get the maximum task_id from all tables
                ...
                # Update the main application's path to use the writable path if needed
                if hasattr(sys, '_MEIPASS') and hasattr(self.main_app, 'path'):
                    self.main_app.path = writable_db_path
                
                # Show success message
                messagebox.showinfo("Success", "Task added successfully!")
                
                # Refresh the main application's task list if possible
                ...
                messagebox.showerror("Database Error", f"Failed to add task to database:\n{e}")
                self.lift()
                self.focus_force()
